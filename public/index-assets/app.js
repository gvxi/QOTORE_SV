...
// Minimal Image Cache Busting - Add to end of index-assets/app.js
// This ONLY adds cache busting without modifying existing functions

(function() {
    'use strict';
    
    // Simple cache buster generator
    function getCacheBuster() {
        return Date.now();
    }
    
    // Add cache buster to image URLs only
    function addCacheBusterToImageSrc(imagePath) {
        ...
        return `/api/image/${cleanPath}?v=${getCacheBuster()}`;
    }
    
    // Store the original createProductCard function if it exists
    const originalCreateProductCard = window.createProductCard;
    const originalOpenProductModal = window.openProductModal;
    
    // Override createProductCard ONLY to add cache busters to images
    if (originalCreateProductCard) {
        window.createProductCard = function(fragrance) {
            // Call original function
            const card = originalCreateProductCard(fragrance);
            
            // Only modify the image src if it exists
            const img = card.querySelector('.product-image img');
            if (img && img.src && img.src.includes('/api/image/')) {
                // Extract the image path and add cache buster
                const pathMatch = img.src.match(/\/api\/image\/([^?]+)/);
                if (pathMatch) {
                    const imagePath = pathMatch[1];
                    img.src = `/api/image/${imagePath}?v=${getCacheBuster()}`;
                }
            }
    ...
    // Override openProductModal ONLY to add cache busters to modal images
    if (originalOpenProductModal) {
        window.openProductModal = function(fragrance) {
            // Call original function first
            originalOpenProductModal(fragrance);
            
            // Then add cache buster to modal image
            const modalImage = document.getElementById('modalImage');
            if (modalImage && modalImage.src && modalImage.src.includes('/api/image/')) {
                const pathMatch = modalImage.src.match(/\/api\/image\/([^?]+)/);
                if (pathMatch) {
                    const imagePath = pathMatch[1];
                    modalImage.src = `/api/image/${imagePath}?v=${getCacheBuster()}`;
                }
            }
        };
    }
    
    // Fallback: If functions don't exist yet, set them up when DOM loads
    document.addEventListener('DOMContentLoaded', function() {
        // Wait a bit for other scripts to load
        setTimeout(function() {
            // If we still don't have the functions, try to override them
            if (typeof window.createProductCard === 'function' && !window.createProductCard.toString().includes('getCacheBuster')) {
                const origCreate = window.createProductCard;
                window.createProductCard = function(fragrance) {
                    const card = origCreate(fragrance);
                    const img = card.querySelector('.product-image img');
                    if (img && img.src && img.src.includes('/api/image/')) {
                        const pathMatch = img.src.match(/\/api\/image\/([^?]+)/);
                        if (pathMatch) {
            ...
            if (typeof window.openProductModal === 'function' && !window.openProductModal.toString().includes('getCacheBuster')) {
                const origModal = window.openProductModal;
                window.openProductModal = function(fragrance) {
                    origModal(fragrance);
                    setTimeout(function() {
                        const modalImage = document.getElementById('modalImage');
