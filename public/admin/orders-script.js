...
// Enhanced order management functions for admin
async function markOrderAsReviewed(orderId) {
    try {
        ...
        if (result.success) {
            showToast(`Order marked as reviewed - customer can no longer cancel`, 'success');
            await refreshData(true);
        } else {
...
async function markOrderAsProcessing(orderId) {
    try {
        console.log('🔄 Marking order as processing:', orderId);
        ...
        if (result.success) {
            ...
            throw new Error(result.error || 'Failed to mark order as processing');
        }
        
    } catch (error) {
        console.error('❌ Failed to mark order as processing:', error);
        showToast('Failed to mark order as processing: ' + error.message, 'error');
    }
}

// Override the existing renderOrderRow function to add new buttons
const originalRenderOrderRow = window.renderOrderRow || function(order) { return ''; };

window.renderOrderRow = function(order) {
    const row = document.createElement('tr');
    row.innerHTML = `
        <td>
            <strong>${order.orderNumber || `ORD-${String(order.id).padStart(5, '0')}`}</strong>
            <br>
        ...
        <td>
            ...
            <div><small>${order.customer.phone}</small></div>
            ${order.customer.email ? `<div><small>${order.customer.email}</small></div>` : ''}
        </td>
        <td>
            <div>${order.delivery.address}</div>
            <div><small>${order.delivery.city}${order.delivery.region ? `, ${order.delivery.region}` : ''}</small></div>
        </td>
        <td>
            <small>${getItemsPreview(order.items)}</small>
            <br>
            <strong>${order.itemCount} item${order.itemCount !== 1 ? 's' : ''}</strong>
        </td>
        <td>
            <strong>${order.total.toFixed(3)} OMR</strong>
        </td>
        <td>
        ...
        <td>
            <div class="action-buttons">
                ...
                ${order.status === 'pending' ? `
                    <button class="btn-small btn-review" onclick="markOrderAsReviewed('${order.id}')" title="Mark as reviewed (prevents customer cancellation)">
                        Mark Reviewed
                    </button>
                    <button class="btn-small btn-processing" onclick="markOrderAsProcessing('${order.id}')" title="Mark as processing">
                        Processing
                    </button>
                ` : ''}
                
                ${order.status === 'pending' || order.status === 'reviewed' || order.status === 'processing' ? `
                    <button class="btn-small btn-complete" onclick="updateOrderStatus('${order.id}', 'completed')" title="Mark as completed">
                        Complete
                    </button>
                ` : ''}
...
function getOrderStatusInfo(order) {
    if (order.status === 'pending') {
        ...
        if (hoursSinceOrder < 1) {
            return `<br><small style="color: #f59e0b;">⚠️ Customer can cancel (${Math.ceil(60 - hoursSinceOrder * 60)} min left)</small>`;
        } else {
            return `<br><small style="color: #10b981;">✅ Cancellation window expired</small>`;
        }
    } else if (order.status === 'reviewed') {
        return `<br><small style="color: #3b82f6;">🔒 Locked from cancellation</small>`;
    } else if (order.status === 'processing') {
        return `<br><small style="color: #8b5cf6;">🔄 Being processed</small>`;
    } else if (order.status === 'completed') {
        return `<br><small style="color: #10b981;">✅ Order completed</small>`;
    } else if (order.status === 'cancelled') {
        return `<br><small style="color: #ef4444;">❌ Order cancelled</small>`;
    }
    return '';
}

// Add CSS for new button styles (add to admin orders CSS)
...
// Enhanced notification for new orders (override existing function)
const originalShowNewOrderNotification = window.showNewOrderNotification || function() {};

window.showNewOrderNotification = function(orderData) {
    // Call original notification
    originalShowNewOrderNotification(orderData);
    
    // Add enhanced notification
    if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('🔔 New Order - Action Required!', {
            body: `Order ${orderData.orderNumber} from ${orderData.customerName}\nMark as "Reviewed" to prevent cancellation`,
            icon: '/icons/icon-192x192.png',
            badge: '/icons/icon-32x32.png',
    ...
    // Show enhanced toast
    showToast(`🔔 New Order ${orderData.orderNumber}! Mark as "Reviewed" to lock from cancellation.`, 'info', 10000);
};

// Export functions for global access
window.markOrderAsReviewed = markOrderAsReviewed;
window.markOrderAsProcessing = markOrderAsProcessing;
window.getStatusDisplayText = getStatusDisplayText;
window.getOrderStatusInfo = getOrderStatusInfo;

console.log('✅ Admin orders enhancement loaded');