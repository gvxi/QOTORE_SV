// functions/admin/toggle-order.js - Toggle order status between pending and completed
export async function onRequestPost(context) {
  ...
  try {
    console.log('Toggle order status request received');
    ...
    const checkResponse = await fetch(`${SUPABASE_URL}/rest/v1/orders?id=eq.${id}&select=id,customer_first_name,customer_last_name,status,total_amount`, {
      headers: {
    ...
    console.log('Found order:', `${order.customer_first_name} ${order.customer_last_name}`, 'Current status:', order.status);
    ...
    return new Response(JSON.stringify({ 
      success: true,
      message: `Order from ${order.customer_first_name} ${order.customer_last_name} marked as ${status}!`,
      data: {
        id: parseInt(id),
...
export async function onRequestGet(context) {
  ...
  return new Response(JSON.stringify({
    message: 'Toggle order status endpoint is working!',
    authenticated: isAuthenticated,
    method: 'POST /admin/toggle-order to change order status',
    requiredFields: ['id (number)', 'status ("pending" or "completed")'],
    examples: {
